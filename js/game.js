...
function Game(canvas, game, gridSizeW, gridSizeH) {
	...
	var nspy = (this.grid.height * this.zoomHelper.level) + this.scrollPosition.y;
	var nspx = (this.grid.width * this.zoomHelper.level) + this.scrollPosition.x;
...
Game.prototype.translatePixelsToMatrix = function(x, y) {
	...
	gridOffsetX += ((this.canvas.width / 2) - zoomedWidth) + this.scrollPosition.x;
